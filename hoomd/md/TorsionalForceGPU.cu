...
__global__ void gpu_compute_torsional_sin_force_kernel(const unsigned int group_size,const BoxDim box,
    ...
    if (group_idx >= group_size)
        return;
    unsigned int idx1 = d_index_array1[group_idx];
    unsigned int idx2 = d_index_array2[group_idx];
    unsigned int idx3 = d_index_array3[group_idx];
    unsigned int idx4 = d_index_array4[group_idx];
    Scalar4 pos_b = __ldg(d_pos + idx1);
    Scalar4 pos_c = __ldg(d_pos + idx2);
    Scalar4 pos_a = __ldg(d_pos + idx3);
    Scalar4 pos_d = __ldg(d_pos + idx4);
