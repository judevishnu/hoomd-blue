...
#ifndef __TORSFORCECOMPUTE_H__
#define __TORSFORCECOMPUTE_H__
...
*/
class PYBIND11_EXPORT TorsForceCompute : public ForceCompute
    {
    public:
    //! Constructs the compute
    TorsForceCompute(std::shared_ptr<SystemDefinition> sysdef, std::shared_ptr<ParticleGroup> group1, std::shared_ptr<ParticleGroup> group2);

    //! Destructor
    virtual ~TorsForceCompute();

    //! Set the table for a given type pair
    virtual void
    setParams(unsigned int type, Scalar k, Scalar d, int n, Scalar phi_0, pybind11::tuple t_q);
    ...
    std::shared_ptr<ParticleGroup> m_group1;       //!< Width of the tables in memory
    std::shared_ptr<ParticleGroup> m_group2;
    Global<Scalar> m_k;
    Global<Scalar> m_d;
    Global<Scalar> m_n;
    Global<Scalar> m_phi_0;
